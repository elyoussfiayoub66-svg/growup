...
export default function ProductsDropdown() {
  ...
  return (
    <nav className="navbar-main">
      {/* Left container: Logo + tagline */}
      <Link to="/" className="navbar-left-container">
        <img src={logo} alt="GrowUp" className="navbar-logo" />
        <div className="navbar-divider"></div>
        <div className="navbar-text">
          <span className="navbar-subtitle">Grow faster</span>
          <span className="navbar-subtitle">Scale smarter.</span>
        </div>
      </Link>
                  ...
                  <div key={idx} className="product-card">
                    ...
                    <ul>
                      {product.features.map((f, i) => (
                        <li key={i}>
  <img src={f.icon} alt="" className={f.class} /> {f.text}
</li>
                      ))}
                    </ul>
                  </div>
        ...
        <div className="nav-dropdown2">
          <button className="nav-link" onClick={() => setSolutionsOpen(!solutionsOpen)}>
         ...
         <div className="nav-dropdown">
          <button className="nav-link" onClick={() => setResourcesOpen(!resourcesOpen)}>
            Resources <ChevronDown size={16} />
          </button>

          {resourcesOpen && (
            <div className="dropdown3-menu">
